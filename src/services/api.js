...
const request = async (method, endpoint, body) => {
  if (isBackendOnline) {
    try {
      const token = localStorage.getItem('authToken'); // set during login
      ...
      // Return the full server JSON so callers like the frontend can read
      // `res.data.success` and `res.data.data` consistently (axios-like shape).
      return { data: json };
    } catch (error) {
      console.warn(
        `Backend request failed: ${endpoint}. Falling back to localStorage.`,
      );
    }
  }
  ...
  if (method === 'PATCH' || method === 'PUT') {
    ...
    const index = data.findIndex(
      (item) => item.id === id || item.task_id === id || item.event_id === id,
    );
    if (index !== -1) {
      ...
      if (endpoint.includes('toggle')) {
        data[index].status =
          data[index].status === 'completed' ? 'pending' : 'completed';
      }
      localStorage.setItem(storageKey, JSON.stringify(data));
...
export const submissionsAPI = {
  // Add '/teacher' to the end to match your router
